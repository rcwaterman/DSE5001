...
Coverage        No    458             727       854     385     99      2,523
                Yes   4,199         6,245     4,821     1,634   578     17,477
Total                 4,657         6,972     5,675     2,019   677     20,000

a.) What is the probability that a single individual drawn at random has excellent
health and does not have Health Insurance?   What would be the standard error on this
estimate of a proportion?

*This cohort represents 458 of the 20000 subjects.*
...
*The standard error is about 0.00106.*

b.)  What is the probability of having Very Good health overall?
...
c.)  what is the probability of having health coverage?
...
d.) What is the probability of having excellent health given that a person has
health coverage?

*P(EX | HC) = P(EX and HC)/P(HC)*

```{r}
P_ex_given_hc = (4199/17477)/P_hc
P_ex_given_hc
```

*About 27.5%*

e.) Are having excellent health and having health coverage independent?
...
*It appears that having excellent health and health care could be independent, as there is a relatively uniform, maybe even slightly normal distribution, of health proportions by coverage status. The excellent proportion does represent the smallest, but it is not significantly smaller.*
...
If we are running a test of a new marketing campaign, of ads A and B.  We decide we want a significance level
of 3% for our sample size of 40 people in each group. 

d.) Are we likely to make a Type 1 error?

*A type 1 error occurs when the null hypothesis is rejected when it is true. A 3% significance level means there is a 3% chance that the null hypothesis has been rejected when it is true, or there is a 3% chance of a type 1 error, making it unlikely.*
...
a.) What test would you use to test the hypothesis that the Sepal.length was more variable in setosa than in versicolor?

*An F-test would be required.*

b.) Run this test in R.   Did the hypothesis hold?

```{r}
var.test(iris[iris$Species=="setosa", 1], iris[iris$Species=="versicolor", 1])
```

*The p-value is less than 0.05, therefore the null hypothesis is rejected, indicating that the variances are different. The hypothesis does hold*

c.) Does the species variable predict differences in Sepal.Width?  What test would you
...
*The species variable strongly predicts differences in Sepal.Width. This is indicated by the near 0 p-value, rejecting the null hypothesis.*
